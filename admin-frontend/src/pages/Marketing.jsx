    ...
    <div>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8, color: '#111827', marginBottom: 4 }}>
          <i className="fa fa-bullhorn" style={{ color: '#7b2020' }} />
      ...
      <div className="stat-card" style={{ marginBottom: 20, padding: 0, overflow: 'hidden' }}>
        <div style={{ display: 'flex' }}>
          <StatBox value={stats.total} label={t.totalRecords} color="#7b2020" />
          <StatBox value={stats.pending} label={t.pending} color="#f59e0b" />
          <div style={{ flex: 1, textAlign: 'center', padding: '16px 8px', borderRight: '1px solid #f3f4f6', borderLeft: '1px solid #e5e7eb', background: '#f9fafb' }} />
      ...
      <div className="stat-card" style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 16, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, marginBottom: 20, color: '#111827' }}>
          <i className="fa fa-cog" style={{ color: '#7b2020' }} />
          {t.marketingSettings}
        </h2>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <button
                  ...
                  style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #e5e7eb', cursor: 'pointer', background: settings.discountType === 'percent' ? '#7b2020' : '#fff', color: settings.discountType === 'percent' ? '#fff' : '#374151', fontWeight: 600 }}
                >%</button>
                <button
                  onClick={() => setSettings(s => ({ ...s, discountType: 'fixed' }))}
                  style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #e5e7eb', cursor: 'pointer', background: settings.discountType === 'fixed' ? '#7b2020' : '#fff', color: settings.discountType === 'fixed' ? '#fff' : '#374151', fontWeight: 600 }}
                >$</button>
                <input
                  className="form-input"
                  type="number"
        ...
        <div style={{ borderTop: '1px solid #f3f4f6', marginTop: 24, paddingTop: 24 }}>
          <h3 style={{ fontSize: 15, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16 }}>
            <i className="fa fa-gift" style={{ color: '#7b2020' }} />
            {t.referralRewards}
          </h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
            <div>
            ...
            <tbody>
              {paged.map(r => (
                <tr key={r.id}>
                  ...
                  <td style={{ fontSize: 13 }}>{r.recipient}</td>
                  <td style={{ fontFamily: 'monospace', fontWeight: 600, color: '#7b2020', fontSize: 13 }}>{r.couponCode}</td>
                  <td style={{ fontSize: 13, color: '#6b7280' }}>{r.triggeredBy}</td>
                  <td>
                  ...
                  <td>
                    {r.couponUsed
                      ? <span className="badge badge-green"><i className="fa fa-check" /> Used</span>
